                	...
                	<div class="product-main">
                        ...
                        <div class="content-product-main">
                            <div class="row">
                                <?php
                                    try {
                                        $stmt = $conn->prepare('SELECT id, image, name, price FROM products WHERE category_id = :cat');
                                        $stmt->execute([':cat' => 2]);
                                        $products = $stmt->fetchAll(PDO::FETCH_ASSOC);

                                        foreach ($products as $kq) {
                                            $pid = (int)$kq['id'];
                                            $img = htmlspecialchars($kq['image'], ENT_QUOTES, 'UTF-8');
                                ...
                                ?>
                                        <div class="col-md-3 col-sm-6 text-center">
                                            <div class="thumbnail">
                                                <div class="hoverimage1">
                                                    <img src="<?= $img; ?>" alt="<?= $name; ?>" width="100%" height="300">
                                                </div>
                                    ...
                                    <?php }
                                        ...
                                        error_log('Product fetch error (fashiongirl): ' . $e->getMessage());
                                    }
                                ?>
                            </div><!-- /row -->
                        </div><!-- /Thời Trang Nữ -->
